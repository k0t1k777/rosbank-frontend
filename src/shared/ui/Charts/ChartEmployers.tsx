...
export default function ChartEmployers() {
  ...
  const numberOfEmployee = employers_plan.period.map((plan) =>
    Number(plan.numberOfEmployee)
  );
  const numberOfBusFactor = employers_plan.period.map((plan) =>
    Number(plan.numberOfBusFactor)
  );
  const numberOfKeyPeople = employers_plan.period.map((plan) =>
    Number(plan.numberOfKeyPeople)
  );
  const labels = employers_plan.period.map((_, index) =>
    (index + 1).toString()
  );
  const employeeData = {
    labels: labels,
